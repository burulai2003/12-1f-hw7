...
const setScrollCheck = () => {
    ...
    const { scrollHeight, clientHeight, scrollTop } = document.
        documentElement;
...
const searchPosts = (event) => {
    ...
    const filteredPosts = dataFromBack.filter((el) =>`${el.title}${el.body}${el.id}`.toLowerCase().indexOf(term)>=0);
    postsContainerEl.innerHTML = filteredPosts.reduce((accum, el) => (accum += renderItem(el)), "")
}
